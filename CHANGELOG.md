...
- **Host checks in URL handling used substring matching**: `LFXProjectResolver.resolve_from_github_url()` accepted any URL merely containing `github.com/`, so `https://elsewhere.example.com/github.com/pwned/repo` resolved to the LFX slug `pwned-repo`; the npm registry check had the same shape. Both now validate the host exactly, reusing `parse_repository_url()` for GitHub URLs. The Swift resolver's `startswith("github.com/")` prefix check now compares the first path segment against `repository.SUPPORTED_HOSTS` exactly, which also makes it case-insensitive. Resolves code scanning alerts 18, 19, 22, 23, and 24.
